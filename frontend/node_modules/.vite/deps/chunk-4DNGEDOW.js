...
async function recoverPublicKey({ hash, signature }) {
  ...
  const { secp256k1: secp256k12 } = await import("./secp256k1-UCSJDCOS.js");
  const signature_ = (() => {
...
function getCreateAddress(opts) {
  const from3 = toBytes(getAddress(opts.from));
  let nonce = toBytes(opts.nonce);
  if (nonce[0] === 0)
    nonce = new Uint8Array([]);
  return getAddress(`0x${keccak256(toRlp([from3, nonce], "bytes")).slice(26)}`);
}
function getCreate2Address(opts) {
  ...
  return getAddress(slice(keccak256(concat([toBytes("0xff"), from3, salt, bytecodeHash])), 12));
}
...
async function simulateCalls(client, parameters) {
  ...
  const getBalanceData = account ? encode(from("constructor(bytes, bytes)"), {
    bytecode: deploylessCallViaBytecodeBytecode,
    args: [
      getBalanceCode,
      encodeData(from2("function getBalance(address)"), [account.address])
    ]
  }) : void 0;
      ...
      ...traceAssetChanges ? [
        ...
        // Asset pre balances
        {
          calls: assetAddresses.map((address, i) => ({
            abi: [
              from2("function balanceOf(address) returns (uint256)")
            ],
      ...
      ...traceAssetChanges ? [
        ...
        // Decimals
        {
          calls: assetAddresses.map((address, i) => ({
            to: address,
            abi: [
              from2("function decimals() returns (uint256)")
            ],
        ...
        // Token URI
        {
          calls: assetAddresses.map((address, i) => ({
            to: address,
            abi: [
              from2("function tokenURI(uint256) returns (string)")
            ],
        ...
        // Symbols
        {
          calls: assetAddresses.map((address, i) => ({
            to: address,
            abi: [from2("function symbol() returns (string)")],
            functionName: "symbol",
            from: zeroAddress,
            nonce: i
          })),
...
async function sendUnsignedTransaction(client, args) {
  var _a, _b, _c;
  const { accessList, data, from: from3, gas, gasPrice, maxFeePerGas, maxPriorityFeePerGas, nonce, to, value, ...rest } = args;
  const chainFormat = (_c = (_b = (_a = client.chain) == null ? void 0 : _a.formatters) == null ? void 0 : _b.transactionRequest) == null ? void 0 : _c.format;
  const format = chainFormat || formatTransactionRequest;
  const request = format({
    // Pick out extra data that might exist on the chain's transaction request type.
...
//# sourceMappingURL=chunk-4DNGEDOW.js.map
